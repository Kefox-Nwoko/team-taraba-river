...
import { validateBody, MemberRegistrationSchema, MemberUpdateSchema, EventCreationSchema, ApprovalDecisionSchema, RSVPSchema, AIQuerySchema, DriveSyncSchema, YouTubeParseSchema, LoginCredentialSchema, AdminAISearchSchema, MediaUploadSchema, MediaFinalizeSchema, } from "./server/validation";
...
setInterval(async () => {
    try {
        ...
        serverLogger.warn("[Firestore Cleanup Worker Notice]", { error: err.message });
    }
...
async function incrementGlobalVisits(visitorName) {
    try {
        if (isFirestoreAvailable()) {
...
app.post("/api/auth/verify", rateLimiter, async (req, res) => {
    ...
    if (!isFirestoreAvailable()) {
        ...
        const member = matched || {
            ...
            skills: ['Development'],
            photoUrl: '', photoStatus: 'approved',
            role: 'admin', activityPoints: 1000,
...
app.get("/api/system/visits", async (req, res) => {
    try {
        let metrics = {
            totalVisits: 0,
            lastVisitTimestamp: new Date().toISOString(),
...
app.post("/api/auth/login", rateLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!emailQuery.empty) {
            memberData = emailQuery.docs[0].data();
            ...
            if (matchedLocal) {
                ...
                if (!phoneQuery.empty) {
                    memberData = phoneQuery.docs[0].data();
...
app.get("/api/members", conditionalAuth, async (req, res) => {
    try {
...
app.post("/api/members", conditionalAuth, async (req, res) => {
...
app.put("/api/members/:id", conditionalAuth, async (req, res) => {
    const { id } = req.params;
...
app.get("/api/events", conditionalAuth, async (req, res) => {
    try {
...
app.post("/api/events", conditionalAuth, async (req, res) => {
...
app.put("/api/events/:id", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
    const { id } = req.params;
    const validation = validateBody(EventCreationSchema, req.body);
    if (!validation.success) {
        res.status(400).json({ error: validation.error });
...
app.delete("/api/events/:id", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const eventRef = db.collection(COLLECTIONS.events).doc(id);
        const eventDoc = await eventRef.get();
...
app.post("/api/events/:id/rsvp", conditionalAuth, async (req, res) => {
...
app.get("/api/admin/approvals", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
    try {
...
app.post("/api/admin/approvals/:id/decision", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
...
app.post("/api/admin/reset-data", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
    try {
        ...
        serverLogger.error("Reset data error", error);
        res.status(500).json({ error: error.message || "Failed to reset data." });
    }
});
// 12. Analytics Service
app.get("/api/admin/analytics", conditionalAuth, async (req, res) => {
    try {
        const members = await getMembers();
        const events = await getEvents();
        let recentLogs;
...
app.post("/api/admin/ai-search", conditionalAuth, conditionalRequireAdmin, async (req, res) => {
...
app.post("/api/media/drive-sync", conditionalAuth, async (req, res) => {
...
app.post("/api/media/cloud-sync-all", conditionalAuth, async (req, res) => {
    try {
        ...
        if (direction === "forward") {
            ...
            for (const event of existingEvents) {
                ...
                if (event.id.startsWith("gdrive_") || event.id === "evt_taraba_gdrive")
                    continue;
                ...
                if (!folderId) {
                    ...
                    try {
                        ...
                        if (isFirestoreAvailable()) {
                            ...
                            if (idx !== -1)
                                fallbackEvents[idx].driveFolderId = folderId;
                        }
                    }
                    catch (err) {
            ...
            if (rootImages.length > 0) {
                const rootImageUrls = rootImages.map((img) => `https://lh3.googleusercontent.com/d/${img.id}`);
                ...
                if (isFirestoreAvailable()) {
                    ...
                    if (idx >= 0)
                        fallbackEvents[idx] = rootEvent;
                    else
                        fallbackEvents.unshift(rootEvent);
                }
        ...
        serverLogger.error("Drive sync error", error);
...
app.post("/api/media/youtube-parse", conditionalAuth, async (req, res) => {
    ...
    if (ytKey && !ytKey.includes("YourYouTubeKey") && ytKey.length > 10) {
        try {
            ...
            serverLogger.warn("YouTube API metadata fetch warning", { error: ytErr.message });
        }
    }
    res.json({
        success: true,
...
app.post("/api/system/save-youtube-key", async (req, res) => {
    ...
    try {
        ...
        serverLogger.warn("Could not write to .env file:", { error: err.message });
    }
...
app.post("/api/media/youtube-back-sync", conditionalAuth, async (req, res) => {
    try {
        ...
        if (explicitIds.length > 0 && ytKey && !ytKey.includes("YourYouTubeKey")) {
            try {
                ...
                serverLogger.warn("Error fetching explicit YouTube video metadata", { error: err.message });
            }
        }
        ...
        if (fetchedVideos.length === 0 && ytKey && !ytKey.includes("YourYouTubeKey") && ytKey.length > 10) {
            try {
                ...
                serverLogger.warn("YouTube Data API channel upload fetch warning", { error: err.message });
            }
        }
        ...
        if (fetchedVideos.length === 0 && targetChannelId) {
            try {
                ...
                serverLogger.warn("YouTube RSS feed error", { error: rssErr.message });
            }
        }
        ...
        for (const vid of fetchedVideos) {
            ...
            const existingMatch = currentEvents.find((e) => e.youtubeVideoUrl?.includes(vid.videoId) || e.title.toLowerCase() === vid.title.toLowerCase());
            if (existingMatch) {
                if (!existingMatch.youtubeVideoUrl) {
                    ...
                    if (isFirestoreAvailable()) {
                        ...
                        if (idx >= 0)
                            fallbackEvents[idx] = existingMatch;
                    }
                    syncedCount++;
                }
            }
        ...
        serverLogger.error("YouTube back-sync error", error);
...
app.post("/api/media/upload", conditionalAuth, async (req, res) => {
...
app.post("/api/media/finalize", conditionalAuth, async (req, res) => {
...
app.get("/api/media/status/:mediaId", conditionalAuth, async (req, res) => {
    try {
...
app.post("/api/ai/query-router", async (req, res) => {
...
6. suggestedActions: list of objects with label, actionType ("NAVIGATE_EVENTS" | "NAVIGATE_MEMBERS" | "OPEN_ASSISTANT" | "VIEW_MEDIA"), and payload.`;
        ...
        serverLogger.error("Gemini AI Query Router Error", error);
        res.status(500).json({
...
app.post("/api/ai/stats-insights", conditionalAuth, async (req, res) => {
...
4. aiConfidence: A number between 0.90 and 1.00.`;
        ...
        serverLogger.error("Gemini AI Stats Insights Error", error);
        res.json({
...
async function fetchLiveExternalNewsItems() {
    const rawItems = [];
    ...
    for (const feed of LIVE_EXTERNAL_FEEDS) {
        try {
            ...
            serverLogger.warn(`External feed fetch warning [${feed.defaultSource}]`, { error: e.message });
        }
        if (rawItems.length >= 25)
            break;
    }
...
function cleanNewsHtmlAndJunk(str) {
    if (!str)
        return "";
    return str
        .replace(/<font[^>]*>/gi, "")
...
async function aiChiefEditorCurate(rawItems) {
    const ai = getGeminiClient();
  ...
  "headlines": [
    {
      ...
      "publishedAt": "Date or Recent"
            ...
            if (Array.isArray(parsed.headlines) && parsed.headlines.length > 0) {
                return parsed.headlines.slice(0, 6).map((h) => ({
            ...
            serverLogger.warn("AI Chief Editor processing warning", { error: e.message });
        }
    }
...
app.get('/api/usosa-news', async (req, res) => {
...
app.post("/api/ai-xplora", async (req, res) => {
    const { query, userName, apiKey } = req.body || {};
    if (!query || typeof query !== "string") {
        return res.status(400).json({ error: "query is required" });
    }
    const customKey = apiKey || req.headers["x-gemini-api-key"];
    ...
    try {
        ...
        serverLogger.error("Gemini Direct Web API Error", error);
        return res.json({
