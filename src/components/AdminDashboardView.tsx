...
interface PendingMediaModerationCardProps {
  req: PhotoApprovalRequest;
  isSelected: boolean;
  onToggleSelect: (id: string) => void;
  onApprove: (req: PhotoApprovalRequest) => void;
  onReject: (req: PhotoApprovalRequest) => void;
  onOpenPreviewModal: (req: PhotoApprovalRequest) => void;
}

const PendingMediaModerationCard: React.FC<PendingMediaModerationCardProps> = ({
  req,
  isSelected,
  onToggleSelect,
  onApprove,
  ...
  const youtubeId = isVideo && req.photoUrl ? extractYouTubeId(req.photoUrl) : null;

  // Desktop hover handler: Autoplay muted video on cursor hover
  ...
  // Mobile touch / click handler: Toggle video playback on touch or click
  const handleCardTouchOrClick = (e: React.MouseEvent | React.TouchEvent) => {
  ...
  return (
    <div
      ...
      className={`group relative bg-white dark:bg-slate-900 p-5 rounded-3xl border transition-all space-y-4 shadow-sm hover:shadow-md cursor-pointer ${
        isSelected
          ? "border-cyan-500 ring-2 ring-cyan-500/50 bg-cyan-50/20 dark:bg-cyan-950/20 shadow-md"
          : "border-slate-200 dark:border-slate-800 hover:border-cyan-500/60"
      }`}
    >
      ...
      <button
        ...
        className="absolute top-4 left-4 z-20 p-1.5 bg-black/75 hover:bg-black/90 backdrop-blur-md rounded-xl text-white transition cursor-pointer flex items-center gap-1.5 shadow-sm"
      ...
      <div className="absolute top-4 right-4 z-20 flex items-center gap-1.5">
        {isVideo && youtubeId && (
          <a
            ...
            className="px-2 py-1 rounded-xl bg-red-600/90 hover:bg-red-600 text-white text-[10px] font-medium transition flex items-center gap-1 shadow-sm"
        ...
        <button
          ...
          className="p-1.5 rounded-xl bg-black/75 hover:bg-black/90 text-slate-300 hover:text-white transition shadow-sm"
      ...
      <div className="w-full aspect-video rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden bg-slate-950 flex items-center justify-center relative shadow-inner">
        {isVideo ? (
          isPlaying ? (
            youtubeId ? (
              <iframe
                src={`https://www.youtube.com/embed/${youtubeId}?autoplay=1&mute=1&controls=1&modestbranding=1&rel=0&loop=1&playlist=${youtubeId}`}
                title={req.folderName || "Video Submission"}
            ...
            <div className="w-full h-full relative flex items-center justify-center">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col items-center justify-center gap-2 p-3 text-center">
                ...
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider bg-black/70 text-white backdrop-blur-md border border-white/20 animate-pulse">
                  Hover (PC) or Tap (Mobile) to Autoplay
                </span>
              </div>
            </div>
          )
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          <h4 className="text-sm text-slate-900 dark:text-white font-semibold truncate">
            {req.memberName || "Community Member"}
          </h4>
          <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase ${
            isVideo ? "bg-red-500/20 text-red-600 dark:text-red-400" : "bg-teal-500/20 text-teal-600 dark:text-teal-400"
          }`}>
            {isVideo ? "Video Clip" : "Photo"}
          </span>
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{req.memberEmail}</p>
        <p className="text-xs text-slate-600 dark:text-slate-300 font-medium truncate pt-0.5">
          📁 {req.folderName || req.adminNotes || "Event Media"}
        </p>
      </div>
  ...
  return (
    <div className="space-y-6 font-sans font-normal">
      ...
      {(() => {
        ...
        return (
          <div className="sticky top-[80px] sm:top-[92px] z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl py-2 sm:py-2.5 -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 border-b border-slate-200/80 dark:border-slate-800 shadow-md transition-colors font-normal">
            {subTabsBar}
          </div>
        );
      })()}
      ...
      {previewModalReq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-fadeIn">
          <div className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
            {/* Header */}
            <div className="p-4 border-b border-slate-800 flex items-center justify-between gap-4 text-white">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                    previewModalReq.type === "video" ? "bg-red-500/20 text-red-400" : "bg-teal-500/20 text-teal-400"
                  }`}>
                    {previewModalReq.type === "video" ? "Video Clip" : "Photo Asset"}
                  </span>
                  <h3 className="text-sm font-semibold truncate">
                    {previewModalReq.folderName || "Media Preview"}
                  </h3>
                </div>
                <p className="text-xs text-slate-400 truncate mt-0.5">
                  Submitted by {previewModalReq.memberName} ({previewModalReq.memberEmail})
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {previewModalReq.type === "video" && extractYouTubeId(previewModalReq.photoUrl) && (
                  <a
                    href={`https://www.youtube.com/watch?v=${extractYouTubeId(previewModalReq.photoUrl)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-semibold flex items-center gap-1.5 transition"
                  >
            ...
            {/* Media Player Stage */}
            <div className="flex-1 min-h-[360px] max-h-[65vh] bg-black flex items-center justify-center overflow-hidden p-2">
              {previewModalReq.type === "video" ? (
                extractYouTubeId(previewModalReq.photoUrl) ? (
                  <iframe
                    src={`https://www.youtube.com/embed/${extractYouTubeId(previewModalReq.photoUrl)}?autoplay=1&controls=1&modestbranding=1&rel=0`}
                    title={previewModalReq.folderName || "Video Preview"}
                ...
                <img
                  src={previewModalReq.previewDataUrl || previewModalReq.photoUrl}
                  alt={previewModalReq.folderName || "Media Preview"}
                  className="w-full h-full object-contain max-h-[60vh] rounded-2xl"
                  onError={handleGoogleDriveImageError}
                />
              )}
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950 flex flex-col sm:flex-row items-center justify-between gap-3">
              <p className="text-xs text-slate-400 text-center sm:text-left">
                {previewModalReq.adminNotes || "Ready for approval into public event media gallery."}
              </p>
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => {
                    const reqToApprove = previewModalReq;
                    setPreviewModalReq(null);
                    handleApprovePhoto(reqToApprove);
                  }}
                  className="flex-1 sm:flex-none px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                >
                  <CheckCircle2 className="w-4 h-4" />
